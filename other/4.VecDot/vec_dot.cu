...
/* partial dot product 注意这里没有求完，只是求解了一个block,256个线程*/
__global__ void dot_stg_1(const FLOAT *x, FLOAT *y, FLOAT *z, int N)
{
    __shared__ FLOAT sdata[256];
    /********************************************************************************
    get thread id: 1D block and 2D grid ，blockDim.x 内置变量 
    #define get_tid() (blockDim.x * (blockIdx.x + blockIdx.y * gridDim.x) + threadIdx.x)  //获取全局的线程ID
    get block id: 2D grid 
    #define get_bid() (blockIdx.x + blockIdx.y * gridDim.x) //获取block在Grid中的ID
    ********************************************************************************/
    int idx = get_tid();//idx为0～N-1
    int tid = threadIdx.x;//threaIdx指明线程所在block中的位置，tid为0~255
    int bid = get_bid();//block指明在Grid中的ID，bid为0~198**198即是grid的大小（注意是二维）
    ...
    /* reduction using shared mem 然后使用规约算法将乘积的结果累加起来*/
...
int main(int argc, char **argv)
{
    ...
    cudaMalloc((void **)&dz, sizeof(FLOAT) * ((N + 255) / 256));//块大小
    ...
    td = get_time();
    /*************** call GPU ***************/
    for (i = 0; i < itr; i++) dot_device(dx, dy, dz, d, N);//为什么是循环20次？？
    /*************** let GPU finish ***************/
    cudaThreadSynchronize();
    td = get_time() - td;
